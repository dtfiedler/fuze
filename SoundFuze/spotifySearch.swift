...
class spotifySearch: NSObject {
                                    ...
                                    if let resultData: NSDictionary = result as? NSDictionary {
                                        ...
                                        if let artistArray = resultData["artists"] as? NSArray {
                                            if let firstArtist: NSDictionary = artistArray[0] as? NSDictionary{
                                                ...
                                                sugresult.artist = artist.stringByTrimmingCharactersInSet(NSCharacterSet.whitespaceCharacterSet())
                                                sugresult.track = title[0].stringByTrimmingCharactersInSet(NSCharacterSet.whitespaceCharacterSet())
                                                sugresult.duration = duration
                                                sugresult.spotifyURL = NSURL(string: sURL.stringByTrimmingCharactersInSet(NSCharacterSet.whitespaceCharacterSet()))!
                                                sugresult.spotifyURI = NSURL(string: sURI.stringByTrimmingCharactersInSet(NSCharacterSet.whitespaceCharacterSet()))!
    ...
    func spotifySearchResultsToStringArray(results: [spotifySearchResult]) -> [String] {
        ...
        for result in results {
            if result.track != "" {
                stringResutls.append((result.track as String) + " (" + (result.artist as String) + ")")
            } else {
                stringResutls.append(result.artist as String)
            }
        }
